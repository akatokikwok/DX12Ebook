...
void CrateApp::BuildDescriptorHeaps()
{
	...
	srvDesc.Texture2D.MipLevels = woodCrateTex->GetDesc().MipLevels;			// 若设为-1,则表示从MostDetailedMip到最后1个mipmap之间的所有mipmap
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;								// 允许访问的最小mipmap,设为0表示可以访问所有的mipmap
...
void CrateApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); ++i) {
		...
		// 各自绑定 关联根常量的视图 并把纹理绑定到流水线,注意mCommandList->SetGraphicsRootConstantBufferView(2, passCB->GetGPUVirtualAddress());
		// 2这个是passcb的,它在Draw()里调用
		cmdList->SetGraphicsRootDescriptorTable(0, tex);			// 0号为纹理
		cmdList->SetGraphicsRootConstantBufferView(1, objCBAddress);// 1号为物体CB
		cmdList->SetGraphicsRootConstantBufferView(3, matCBAddress);// 3号为材质CB
