...
private:
	...
	UINT mSkyTexHeapIndex = 0;// 天空球在堆中的视图 序号
...
void CubeMapApp::Draw(const GameTimer& gt)
{
	...
	skyTexDescriptor.Offset(mSkyTexHeapIndex, mCbvSrvDescriptorSize);// 视图在堆中的序数 由BuildDescriptorHeaps()编辑
	mCommandList->SetGraphicsRootDescriptorTable(3, skyTexDescriptor);// 管线上绑定天空球的 SRV描述符表,设定为3号

	// 设置描述符表，将纹理资源与流水线绑定(因为只绑定一次，所以不需要做地址偏移),设定为4号
...
void CubeMapApp::LoadTextures()
{
	...
	// 给所有的纹理构造各自的纹理 并在 全局纹理字段里注册
	for (int i = 0; i < (int)texNames.size(); ++i) {
		auto texMap = std::make_unique<Texture>();
		texMap->Name = texNames[i];
		texMap->Filename = texFilenames[i];
		// CreateDDSTextureFromFile12接口来创建贴图
...
void CubeMapApp::BuildRootSignature()
{
	...
	CD3DX12_DESCRIPTOR_RANGE texTable0;// cubuemap
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1/*描述符数量（纹理数量*/, 0/*描述符所绑定的寄存器槽号*/, 0);
	CD3DX12_DESCRIPTOR_RANGE texTable1;// 2d texture
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 5/*描述符数量（纹理数量*/, 1/*描述符所绑定的寄存器槽号*/, 0);

	// Root parameter can be a table, root descriptor or root constants.
	CD3DX12_ROOT_PARAMETER slotRootParameter[5];
	// 由于CubeMap资源和2D纹理资源是不同类型数据，在Shader中有不同的数据类型表示（TextureCube和Texture2D)
	// 所以在传输到GPU时要分开处理，分别绑定两个根签名
	// Perfomance TIP: Order from most frequent to least frequent.
	slotRootParameter[0].InitAsConstantBufferView(0);// objCB绑定槽号为0的寄存器,但是space在0号
	slotRootParameter[1].InitAsConstantBufferView(1);// passCB绑定槽号为1的寄存器
	slotRootParameter[2].InitAsShaderResourceView(0, 1);// matSB也绑定槽号为0的寄存器,但是space却在1号（和纹理公用一个SRV寄存器，但是不同Space）
	// cubemap纹理 和 2D纹理 绑定了两个srvTable
	slotRootParameter[3].InitAsDescriptorTable(1/*Range数量*/, &texTable0/*Range指针*/, D3D12_SHADER_VISIBILITY_PIXEL/*该资源只能在像素着色器可读*/);
...
void CubeMapApp::BuildDescriptorHeaps()
{
	...
	// next descriptor
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;// 改成D3D12_SRV_DIMENSION_TEXTURECUBE，其他都不变
	...
	mSkyTexHeapIndex = 3;// 暂存天空球在堆中的视图 序号
}
...
void CubeMapApp::BuildShadersAndInputLayout()
{
	...
	// 天空球和场景中物件是分开绘制的，而且天空球Shader不需要光照，并且采样方法也不同，所以要使用单独的shader，我们增加一组shader用来绘制天空球。
...
void CubeMapApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData sphere = geoGen.CreateSphere(0.5f, 20, 20);// 构造出天空球模型
	GeometryGenerator::MeshData cylinder = geoGen.CreateCylinder(0.5f, 0.3f, 3.0f, 20, 20);

	/// 依次暂存 各个mesh的在全局中的 顶点偏移和索引偏移
	...
	/// 利用全局里的顶点偏移和索引偏移 构造子几何Mesh的 三绘制参数
	...
	std::vector<Vertex> vertices(totalVertexCount);// 顶点集
	...
	geo->VertexByteStride = sizeof(Vertex);// 单顶点大小
...
void CubeMapApp::BuildPSOs()
{
	...
	//
	// PSO for sky.
	// 由于shader的不同，所以我们必须单独创建“天空球”的PSO，除了设置单独的shader以外
	// 还修改了深度比较函数（远裁剪面是天空球，所以必须<=，天空球才不会被裁剪），并且天空球是只看内部的，所以关闭剔除（也可以CULL_FRONT
	D3D12_GRAPHICS_PIPELINE_STATE_DESC skyPsoDesc = opaquePsoDesc;
	skyPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;// 天空球是只看内部的，所以关闭剔除（也可以CULL_FRONT
...
void CubeMapApp::BuildMaterials()
{
	...
	auto sky = std::make_unique<Material>();// 因为天空球材质是和其他的材质共享同一个SRV堆，所以只需将Index排在其他纹理之后即可
	sky->Name = "sky";
	sky->MatCBIndex = 4;
	sky->DiffuseSrvHeapIndex = 3;
	sky->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);// 天空球应该是自发光，所以Albedo为1
